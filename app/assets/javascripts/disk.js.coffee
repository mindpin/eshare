class PartUpload
  ...
  upload_next_chunk: ->
    ...
    # outstanding = false
    # jQuery(@files).each (index, file) =>
    #   outstanding = false
    #   jQuery(file.chunks).each (index1, chunk) =>
    #     status = chunk.status()
    #     if status == 'pending' || status == 'uploading' || chunk.preprocess_state == 1
    #       outstanding = true
    #       return false
...
jQuery ->
  ...
  pu.assign_button jQuery('a.upload')

  pu.on 'fileAdded', (file) ->
    # console.log pu
    # jQuery(pu.files[0].chunks).each (i, c) ->
    #   console.log c.start_byte, c.end_byte
    pu.upload()

  pu.on 'fileError', (file, message) ->
    # console.log message
    # jQuery(pu.files[0].chunks).each (i, c) ->
    #   console.log c.status()

  pu.on 'fileProgress', (file) ->
