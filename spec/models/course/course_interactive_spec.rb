...
describe CourseInteractive do
  before{
    ...
    Question.create(:title=>"title_1", :content => 'content_1', :creator => @user, :chapter => @chapter_1)
    Question.create(:title=>"title_2", :content => 'content_2', :creator => @user, :chapter => @chapter_2)

    Timecop.travel(Time.now - 2.day) do
      Question.create(:title=>"title_3", :content => 'content_3', :creator => @user, :chapter => @chapter_3)
    end
    ...
    describe '逐步增加课程交互数' do
      ...
      describe '创建指定了章节的问题' do
        before{
          ...
          Question.create(:title=>"title_new", :content => 'content_new', :creator => @user, :chapter => @new_chapter)
        }
